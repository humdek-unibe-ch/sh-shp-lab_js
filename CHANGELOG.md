...
### New feature
 ...
 - `preload_files` on the `labJS` style: every file in the experiment pool is fetched in the background once the experiment starts, so a later screen shows its stimulus from the browser cache rather than waiting on the network. lab.js only preloads `options.media`, which a study built from a file pool does not set, so until now the first sight of an image cost a round trip in the middle of a trial. The fetch takes a few files at a time and starts after the experiment is running, so it never delays the screen the participant is on. Off by default. Worth pairing with a cache lifetime on the files themselves: a response carrying only `ETag`/`Last-Modified` is revalidated, so each later screen still pays a conditional request
